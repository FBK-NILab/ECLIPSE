...
class HCPData(Dataset):
    ...
    def __init__(self, base_path, mode='train', streamline_number=1000, padding=True, max_streamline_length=0, **kwargs):
        ...
        self.col_min = np.array([-67.97412872314453, -106.12793731689453, -72.49878692626953], dtype=np.float32)
        self.col_max = np.array([69.92117309570312, 71.78073120117188, 79.93974304199219], dtype=np.float32)
    ...
    def __getitem__(self, item):
        ...
        if not self.return_geometric:
            lens = np.minimum(lens, self.max_streamline_length)
            if self.padding:
                new_lines = list()
                masks = list()
                for idx, line in enumerate(lines):
                    line = line[:self.max_streamline_length]
                    last_point = line[-1]
