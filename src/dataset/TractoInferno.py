...
class TractoInfernoData(Dataset):
    ...
    def __init__(self, base_path, mode='train', streamline_number=1000, padding=True, max_streamline_length=0, **kwargs):
        ...
        if self.mode == 'test':
            self.streamline_number = 2000000
        ...
        self.pattern_to_check = kwargs.get('pattern', 'det')
        ...
        # Edge index cache: map length -> torch.LongTensor
    ...
    def set_pattern(self, val):
        assert val in ['det', 'prob', 'pft', 'set']
        self.pattern_to_check = val
    ...
    def __getitem__(self, item):
        ...
        sub_trk_path = os.path.join(self.base_path, self.subs[item], 'tractograms')
        trk_file = [item for item in os.listdir(sub_trk_path) if ('trk' in item and self.pattern_to_check in item)][0]
        ...
        lines, lens = load_streamlines(trk_file, idxs=self.streamline_number,
                                       ...
                                       plausible_only=False, non_plausible_only=False,
                                       labels=None, mode=self.mode)
        num_lines = len(lines)
